...
Instead of $U$ updating the synapse directly, $U$ acts as a transcription factor for a target gene inside each neuron. The gene produces a structural protein $P$.

$$\frac{dP_i}{dt} = \alpha \frac{U_i^m}{K_{GRN}^m + U_i^m} - \beta P_i$$
...
Now we update the connection weight $W_{ji}$ (from pre-synaptic neuron $j$ to post-synaptic neuron $i$). We combine our Hebbian learning idea with the new protein $P$ and a spatial energy penalty.

$$\Delta W_{ji} = \eta \cdot \left[ P_i \cdot S_j \cdot e^{-\lambda d_{ji}} \right] - \delta W_{ji}$$
