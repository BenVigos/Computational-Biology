...
If we look at Figure A we see that the addition of angiogenesis allows for growth after the first initial spike, while the model without angiogenesis reaches a steady state. Subplot b shows that the peak around 900 MCS is caused by a significant spike in nectoric cells. If we look at figure B, subplot c we see that this point corresponds with an increase of oxygen from the reached equillibrium, due to the approaching endothelial cells. This supply of oxygen allows for further tumor growth, which eventually fills almost the entire domain.

Figure A, subplot c shows that with angiogenesis the hypoxic fraction continuiously declines after the second peak, while the model without angiogenesis seems to oscillate at a higher value. The supply of fresh oxygen through endothelial cells does not just allow for growth, it allows for healthier growth with less hypoxic cells.

Figure B, subplot a, shows the mean VEGF and HIF-1 alpha concentrations inside the tumor cells. We see that addition of HIF-1 alpha has very little effect on the VEGF concentrations. However, hen we look at the activation of neovascular cells in subplot b, we see a significant increase in the amount of active neovascular cells. During the initialization there is some factor causing an increase in inactive neovascular cells, which allows for this increased level of activated cells. We have not found the cause within our model for this incrase of inactive neovascular cells due to time constraints.

Figure C, subplot b, shows that HIF-1a causes an earlier release of VEGF within the tumor, allowing for earlier neovascular cell activation as we saw in B, subplot b. After the peaks at 1200 Monte Carlo Steps we see a decrease in both VEGF and HIF-1a with angiogenesis due to the fresh supply of oxygen and accompanying decrease in hypoxic cells.

Finally Figure C, subplot c, shows that after the angiogenesis moves the endothelial cells towards the tumor, the oxygen levels rise which cause the tumor to grow. 
